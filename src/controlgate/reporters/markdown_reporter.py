...
class MarkdownReporter:
    ...
    def render(self, verdict: Verdict) -> str:
        ...
        """
        emoji = _STATUS_EMOJI.get(verdict.verdict, "❓")
        lines = [
            f"# {emoji} ControlGate Verdict: **{verdict.verdict}**",
            "",
            f"> **Framework**: {'FedRAMP' if verdict.is_gov else 'NIST'} · **Baseline**: {verdict.baseline_target} · **Scan**: {verdict.timestamp}",
            "",
        ...
        if not block_findings and not warn_findings:
            lines.append("## ✅ All clear — no security issues detected")
            lines.append("")

        lines.append("---")
        framework_str = "FedRAMP" if verdict.is_gov else "NIST SP 800-53 Rev. 5"
        lines.append(
            f"*Generated by [ControlGate](https://github.com/controlgate) · {framework_str}*"
        )
